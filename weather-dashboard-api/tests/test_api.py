...
def test_get_city_success(client: TestClient) -> None:
    resp = client.get(f"/api/cities/{_CITY_BERLIN}")
    assert resp.status_code == 200
    data = resp.json()
...
def test_delete_weather_record(client: TestClient) -> None:
    payload = {
        "city_id": _CITY_BERLIN,
        "temperature_c": 5.0,
        "humidity": 60,
    ...
    create_resp = client.post("/api/weather", json=payload)
    assert create_resp.status_code == 201
    record_id = create_resp.json()["id"]

    delete_resp = client.delete(f"/api/weather/{record_id}")
    assert delete_resp.status_code == 204

    repeat_delete_resp = client.delete(f"/api/weather/{record_id}")
    assert repeat_delete_resp.status_code == 404
